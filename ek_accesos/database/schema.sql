...
CREATE TABLE IF NOT EXISTS `empleados` (
    ...
    `nombre`           VARCHAR(200) NOT NULL,
    `apellido_paterno` VARCHAR(100) NULL COMMENT 'Apellido paterno',
    `apellido_materno` VARCHAR(100) NULL COMMENT 'Apellido materno',
    `puesto`           VARCHAR(150) NULL,
    `empresa_id`       INT UNSIGNED NULL,
    `jefe_id`          INT UNSIGNED NULL COMMENT 'Jefe directo para organigrama',
    `email`            VARCHAR(150) NULL COMMENT 'Correo operativo',
    `telefono`         VARCHAR(30)  NULL,
    `password_ek`      VARCHAR(255) NULL COMMENT 'Contraseña ERP cifrada (solo admin)',
    `pin_ek`           VARCHAR(255) NULL COMMENT 'NIP ERP cifrado (solo admin)',
    `aprobado`         TINYINT(1)   NOT NULL DEFAULT 1 COMMENT '0=pendiente (capturista), 1=aprobado',
    `creado_por`       INT UNSIGNED NULL COMMENT 'ID del usuario que creó el registro',
    `activo`           TINYINT(1)   NOT NULL DEFAULT 1,
    `created_at`       TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
    `updated_at`       TIMESTAMP    DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`       TIMESTAMP    NULL,
