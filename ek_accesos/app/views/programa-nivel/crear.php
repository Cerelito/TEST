...
// Count total modules (none granted in create mode)
function countModulesCrear(array $nodes): int {
    $total = 0;
    foreach ($nodes as $n) {
        if (empty($n['es_separador'])) $total++;
        if (!empty($n['children'])) $total += countModulesCrear($n['children']);
    }
    return $total;
}
$totalMods = countModulesCrear($arbol);

function renderTreeNodeCrear(array $node, int $depth = 0): void {
    ...
    <div class="pn-row" data-label="<?php echo strtolower(strip_tags($label)); ?>" style="padding-left:<?php echo 8 + $depth * 20; ?>px;">
        ...
        <div class="pn-children" id="<?php echo $nodeId; ?>_ch">
            <?php foreach ($node['children'] as $child): renderTreeNodeCrear($child, $depth + 1); endforeach; ?>
        </div>
        <?php endif; ?>
    </div>
    <?php
...
<div class="pn-header">
    <div>
        <h2 class="pn-header-title">Nuevo Programa Nivel</h2>
        ...
        <div class="form-row form-row-2">
            ...
            <div class="form-group">
                ...
                <input type="number" id="nivel" name="nivel" class="form-control"
                    ...
                    value="<?php echo htmlspecialchars($_POST['nivel'] ?? ''); ?>">
            </div>
        </div>
        <div class="form-group" style="margin-bottom:0;">
            <label class="form-label" for="descripcion">Descripción</label>
            <textarea id="descripcion" name="descripcion" class="form-control" rows="3"
                placeholder="Descripción breve de este nivel de programa..."><?php echo htmlspecialchars($_POST['descripcion'] ?? ''); ?></textarea>
        </div>
    </div>
    ...
    <div class="form-card">
        ...
        <div class="pn-toolbar">
            ...
            <div class="pn-counter" id="pnCounter">
                <span id="pnGranted">0</span> / <span><?php echo $totalMods; ?></span> módulos seleccionados
            </div>
        ...
        <div class="pn-root-block" id="rootblock_<?php echo (int)$root['id']; ?>">
            ...
            <div class="pn-root-body">
                <?php renderTreeNodeCrear($root, 0); ?>
            </div>
        </div>
        <?php endforeach; ?>
        </div>
...
(function () {
    ...
    function cascadeDown(item, state) {
        ...
        children.querySelectorAll('.pn-check').forEach(function (c) {
            c.checked = state; c.indeterminate = false;
        });
    }
            ...
            document.querySelectorAll('.pn-root-block').forEach(function (block) {
                ...
                rows.forEach(function (row) {
                    ...
                    if (lbl.indexOf(q) !== -1) {
                        ...
                        blockVisible = true; anyVisible = true;
