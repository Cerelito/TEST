...
<div class="mb-4 d-flex gap-2 align-items-center">
  <a href="<?= BASE_URL ?>/requisitores" class="btn btn-glass btn-sm">← Requisitores</a>
  <a href="<?= BASE_URL ?>/empleados/<?= (int)($req['empleado_id'] ?? 0) ?>" class="btn btn-glass btn-sm">Ver empleado completo</a>
</div>
...
<div class="glass" style="padding:20px 24px;border-radius:var(--radius-lg);margin-bottom:20px;display:flex;align-items:center;gap:20px;">
  <div style="width:56px;height:56px;border-radius:14px;background:linear-gradient(135deg,#10b981,#06b6d4);
              display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;color:#fff;flex-shrink:0;">
    <?= strtoupper(substr($req['nombre'] ?? 'R', 0, 1)) ?>
  </div>
  <div style="flex:1;">
    <h2 style="font-size:20px;font-weight:800;margin-bottom:2px;"><?= htmlspecialchars($req['nombre'] ?? '') ?></h2>
    <div style="font-size:13px;color:#64748b;">
      <?= htmlspecialchars($req['puesto'] ?? '—') ?>
      <?php if (!empty($req['empresa_nombre'])): ?>
       · <span style="color:#818cf8;"><?= htmlspecialchars($req['empresa_nombre']) ?></span>
      <?php endif; ?>
    </div>
  </div>
  <div>
...
<!-- Centros de Costo con permisos REQ -->
<div class="glass" style="border-radius:var(--radius-lg);overflow:hidden;">
  <div style="padding:16px 20px;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;align-items:center;justify-content:space-between;">
    <h3 style="font-size:15px;font-weight:700;margin:0;">Centros de Costo — Requisiciones</h3>
    <span class="badge badge-purple"><?= count($cc) ?> CC</span>
  </div>
  <?php if (empty($cc)): ?>
  <div style="padding:40px 20px;text-align:center;color:#64748b;">
    <p>Sin centros de costo de tipo REQ asignados.</p>
  </div>
  <?php else: ?>
